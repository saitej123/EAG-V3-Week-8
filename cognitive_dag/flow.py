...
class Graph:
    ...
    def _insert_critic_on_edge(self, src_id: str, child_id: str) -> str | None:
        ...
        critic_spec = NodeSpec(skill="critic", inputs=[src_id], metadata=critic_meta)
    ...
    def _splice_critics_for_added(self, node_ids: list[str]) -> list[str]:
        """Insert critic on edges from critic:true producers to their new children."""
        inserted: list[str] = []
        for src_id in list(node_ids):
            ...
            for child_id in list(self.dg.successors(src_id)):
                if child_id not in node_ids and child_id not in self.dg.successors(src_id):
                    continue
...
class Executor:
    ...
    async def _handle_node_failure(
        ...
        f_label = str(self.graph.dg.nodes[node_id].get("label") or node_id)
        recovery = NodeSpec(
            skill="planner",
            inputs=["USER_QUERY", f"n:{f_label}"],
            metadata={
