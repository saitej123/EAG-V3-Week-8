...
from cognitive_dag.dag_schemas import NodeSpec, NodeStatus, PlannerOutput
from cognitive_dag.flow import Graph
from cognitive_dag.persistence import SessionStore, node_filename
...
def test_critic_splice_yields_ready_critic_node():
    ...
    from cognitive_dag.dag_schemas import NodeState
...
def test_format_memory_hits_preview():
    from cognitive_dag.schemas import MemoryItem
    from cognitive_dag.skills import format_memory_hits
...
def test_session_store_atomic_roundtrip(tmp_path, monkeypatch):
    ...
    from cognitive_dag.dag_schemas import AgentResult
...
def test_prime_memory_read_uses_empty_history_and_top_k(monkeypatch):
    ...
    monkeypatch.setattr("cognitive_dag.flow.format_memory_hits", lambda hits: "")

    async def _sync_to_thread(fn, *a, **kw):
        return fn(*a, **kw)

    monkeypatch.setattr("cognitive_dag.flow.asyncio.to_thread", _sync_to_thread)
