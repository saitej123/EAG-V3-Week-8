...
from cognitive_dag.catalog import get_dag_query, load_worked_queries, worked_queries_payload
...
def test_query_a_matches_base_query_a_text():
    ...
    base_a = next((q for k, q, _ in load_base_queries() if k.lower() == "a"), None)
    assert dag_a is not None and base_a is not None
    assert dag_a["query"] == base_a
