...
**Where it lives:** `cognitive_dag/memory.py` — vector-first `read()` with keyword overlap as fallback only when embeddings or the index are unavailable.
...
**Where it lives:** `cognitive_dag/indexing.py` — `_chunk_text()`, `index_document_path()`.
...
**Where it lives:** `cognitive_dag/memory.py` — `_load_faiss_from_disk()` on every `read()`.
...
**Where it lives:** `cognitive_dag/llm_env.py` — `gemini_embed_model()`, `try_embed_text()`.
